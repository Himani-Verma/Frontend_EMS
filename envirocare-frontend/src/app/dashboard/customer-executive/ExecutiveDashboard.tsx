...
export default function ExecutiveDashboard() {
  ...
  useEffect(() => {
    ...
    console.log('🔍 CustomerExecutiveDashboard: Loading user data...', { userStr: userStr ? 'Found' : 'Not found' });
    
    if (userStr) {
      try {
        const userData = JSON.parse(userStr);
        console.log('👤 CustomerExecutiveDashboard: User data loaded:', { name: userData.name, role: userData.role });
        setUser(userData);
        ...
        if (!['executive', 'sales-executive', 'customer-executive'].includes(userData.role)) {
          console.warn(`User ${userData.name} (${userData.role}) attempted to access executive route`);
      ...
      console.log('❌ CustomerExecutiveDashboard: No user data found, redirecting to login');
      router.push('/login');
      return;
    }
